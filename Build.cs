...
switch (cmd)
{
    case "bench":
        Run(
            "dotnet",
            "run --project src/Maple.Enums.Benchmarks/Maple.Enums.Benchmarks.csproj"
    ...
    case "comparison-bench":
        Run(
            "dotnet",
            "run --project src/Maple.Enums.ComparisonBenchmarks/Maple.Enums.ComparisonBenchmarks.csproj"
                + " -c Release -- run",
            repoRoot
        );
        break;
    ...
    case "disasm":
        Run(
            "dotnet",
            "run --project src/Maple.Enums.Benchmarks/Maple.Enums.Benchmarks.csproj"
                + " -c Release -- --filter \"*Maple.Enums*\" --disasm",
            repoRoot
        );
        break;
    ...
    case "pack":
        ...
        // so only the library produces .nupkg files.
    ...
    case "publish-tester":
        ...
        Run(
            "dotnet",
            $"publish src/Maple.Enums.Tester/Maple.Enums.Tester.csproj -c Release -r {rid} --self-contained",
            repoRoot
        );
        break;
...
static void RenameAll(string repoRoot, string oldName, string newName)
{
    ...
    foreach (var file in files)
    {
        ...
        {
            continue;
        } // skip binary or otherwise unreadable files
